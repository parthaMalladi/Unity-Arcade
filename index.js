...
app.get("/discussion", async (req, res) => {
  ...
  res.render("discussion.ejs", {loggedIn : isLoggedIn, details : user, forum : posts.rows});
});

app.post("/discussion", async (req, res) => {
  const post = req.body;
  try {
    await db.query("INSERT INTO forum (title, post, user_id) VALUES ($1, $2, $3)",
      [post.title, post.msg, user[0].id]
    );
...
app.post("/deleteDiscussion", async (req, res) => {
  const id = req.body.postId;
  try {
    await db.query("DELETE FROM forum WHERE id = $1", [id]);
    res.redirect("/discussion");
  } catch (err) {
    console.log(err);
  }
